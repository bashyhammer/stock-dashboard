        ...
        for symbol in tickers:
            try:
                ...
                if history.empty or len(history) < 2:
                    print(f"Skipping {symbol}: Delisted or insufficient data.")
                    continue
                ...
                formatted_vol = f"{int(volume):,}" if volume and not math.isnan(volume) else "N/A"
...
def get_stock_history(symbol: str):
    """Endpoint to fetch 30-day price history for the modal trend line chart"""
    try:
        stock = yf.Ticker(symbol)
